...
int main(){
    ...
    tempIndex = treeSearch(main_tree, tempNode.key);
    if(tempIndex != -1){ // if the node is in the tree
        tempIndex = treeSuccessor(main_tree, tempIndex); // search for its successor
        if(tempIndex != -1){
            for(int i = 0; i < SIZE; i++){
                if(main_tree.arrNodes[i].key != tempNode.key && treeSearch(main_tree, tempNode.key) != tempIndex){
                    assert(main_tree.arrNodes[i].key > main_tree.arrNodes[tempIndex].key); // any key other than the original key and the successor has to be strictly larger than the successor
                }
            }
        }
        else{ // assert there is no successor
            for(int i = 0; i < SIZE; i++){
                assert(tempNode.key >= main_tree.arrNodes[i].key); // if there is no successor, the original key is the max key
            }
        }
    }
