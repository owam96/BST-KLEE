...
treeNode* treeDeleteNode(treeNode* ptr_tree_root, treeNode* ptr_node){
    ...
    treeNode* x;
    if(ptr_node == NULL){
        fprintf(stderr, "Cannot delete a null node!\n");
        exit(-1);
    }
    if(treeSearch(ptr_tree_root, ptr_node->key) == NULL){
        fprintf(stderr, "Node to delete is not in tree!\n");
        exit(-1);
    }
...
int main(){
    ...
    for(int i = 0 ; i < 3; i++){
        treeRoot = treeDeleteNode(treeRoot, arrNodes[i]);
    }
