...
int main(){
    ...
    int32_t inorderKeys[TREE_SIZE];
    memset(inorderKeys, -1, sizeof(inorderKeys));
    inorderTreeWalk(main_tree, inorderKeys);
    for(int i = 0; i < TREE_SIZE-1; i++){
        if(inorderKeys[i] != -1 && inorderKeys[i+1] != -1)
            assert(inorderKeys[i] <= inorderKeys[i+1]);
    }
    ...
    // assert tree is empty after deletion of all nodes
    for(int i = 0; i < TREE_SIZE; i++){
        assert(main_tree.arrNodes[i].key == -1);
    }
