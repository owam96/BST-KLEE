...
int main(){
    ...
    origIndex = treeSearch(main_tree, tempNode.key);
    if(origIndex != -1){ // if the node is in the tree
        succIndex = treeSuccessor(main_tree, tempIndex); // search for its successor
        if(succIndex != -1){
            for(int i = 0; i < SIZE; i++){
                tempIndex = treeSearch(main_tree, symbolic_array[i]);
                if(main_tree.arrNodes[tempIndex].key != tempNode.key && tempIndex != succIndex){
                    tempIndex = treeSearch(main_tree, symbolic_array[i]);
                    if(tempIndex != -1){ // If key was actually inserted in tree
                        assert(!(main_tree.arrNodes[tempIndex].key < main_tree.arrNodes[succIndex].key && main_tree.arrNodes[tempIndex].key > main_tree.arrNodes[origIndex].key)); // no key is less than successor AND larger than original key
                    }
                }
            }
        }
